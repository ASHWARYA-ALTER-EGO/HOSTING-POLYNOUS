...
export default function App() {
  ...
  const handleLogin = (data) => {
    if (data.skip) {
      setIsLoggedIn(true)
      setUser({ username: 'Guest', email: 'guest' })
    } else {
      setIsLoggedIn(true)
      setUser({ username: data.username || 'User', email: data.email || '' })
    }
  }

  const handleLogout = () => {
    localStorage.removeItem('polynous_token')
    localStorage.removeItem('polynous_user')
    setIsLoggedIn(false)
    setUser(null)
    setMessages([])
  }
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            try {
              ...
              else if (data.type === 'token') setStreamingContent((prev) => prev + (data.content || ''))
        ...
        setMessages((prev) => {
          ...
          last.content = streamingContent; last.sources = assistantMsg.sources
    ...
    <>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '20px 20px 40px', position: 'relative', zIndex: 1, fontFamily: 'Inter, Segoe UI, system-ui, sans-serif' }}>
        ...
        <div style={{ textAlign: 'center', padding: '30px 20px 20px' }}>
          <div style={{ fontSize: '11px', color: '#555', letterSpacing: '4px', textTransform: 'uppercase', marginBottom: '14px' }}>Multi-Agent Research OS</div>
          ...
          <p style={{ color: '#8899aa', maxWidth: '600px', margin: '0 auto 20px', lineHeight: 1.6, fontSize: '14px' }}>
            7-agent cognitive architecture. Real-time streaming. Self-critiquing memory that cross-references, debates, and reasons like a research team.
          </p>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '16px' }}>
            <button onClick={() => document.getElementById('chat')?.scrollIntoView({ behavior: 'smooth' })} style={{ padding: '12px 28px', borderRadius: '30px', border: 'none', background: '#00ff0f', color: '#0a0a1a', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>Get Started</button>
            <button style={{ padding: '12px 28px', borderRadius: '30px', border: '1px solid #00ff0f', background: 'transparent', color: '#00ff0f', fontWeight: 600, fontSize: '14px', cursor: 'pointer' }}>View on GitHub</button>
          </div>
          <code style={{ padding: '10px 22px', borderRadius: '10px', background: '#0f0f1a', border: '1px solid #1e1e2a', fontSize: '13px', color: '#00ff0f', fontFamily: 'monospace' }}>
            <span style={{ color: '#555' }}>$ </span>npm run polynous
          </code>
        </div>

        {/* STATUS BAR */}
        <div style={{ display: 'flex', justifyContent: 'center', gap: '14px', padding: '8px 20px', background: 'rgba(255,255,255,0.02)', borderRadius: '30px', marginBottom: '18px', border: '1px solid rgba(255,255,255,0.05)', fontSize: '11px', color: '#555', flexWrap: 'wrap' }}>
        ...
        {loading && agentProgress.length > 0 && (
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '14px', padding: '14px 20px', marginBottom: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ color: '#f093fb', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '8px', marginTop: '6px' }}><span style={{ animation: 'spin 1s infinite' }}>⚡</span> {agentStatus}</div>
          </div>
        )}
