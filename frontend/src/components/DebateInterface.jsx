...
// ─── COLLAPSIBLE SIDEBAR WITH PARENT STATE ──────────────────────────────
    ...
    <div style={{ minHeight: "100vh", background: C.void, position: "relative", overflow: "auto" }}>
      ...
      <main style={{ 
        ...
        <div style={{ textAlign: "center", marginBottom: 30, paddingTop: 10 }}>
          <h1 style={{ fontFamily: "'Sora',sans-serif", fontSize: "clamp(1.6rem,4vw,2.2rem)", fontWeight: 800, color: C.crimson, margin: "0 0 4px", letterSpacing: "-0.02em" }}>⚖️ Debate Chamber</h1>
          <p style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: C.textSecondary, textTransform: "uppercase", letterSpacing: "3px" }}>Neural Argument Synthesis</p>
        </div>

        {/* Proposition Input */}
        {!debateResult && (
          <div style={{ background: "rgba(10,10,30,0.6)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,32,64,0.2)", borderRadius: 20, padding: 32, marginBottom: 24, boxShadow: "0 0 30px rgba(255,32,64,0.1)" }}>
            ...
            <form onSubmit={startDebate} style={{ display: "flex", gap: 12 }}>
              ...
              <button type="submit" disabled={loading || !topic.trim()} style={{ padding: "16px 32px", borderRadius: 30, border: "none", background: loading ? "#333" : C.crimson, color: "#fff", fontWeight: 700, fontFamily: "'Sora',sans-serif", fontSize: 14, cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : "0 0 25px rgba(255,32,64,0.3)", transition: "all 0.3s" }}>
                {loading ? agentStatus || "Debating..." : "⚖️ Begin Debate"}
              </button>
            </form>
            {/* Suggested topics */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 16 }}>
              {suggestedTopics.map((t, i) => (
                <button key={i} onClick={() => setTopic(t)} style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid rgba(255,32,64,0.2)", background: "rgba(255,32,64,0.04)", color: C.crimson, cursor: "pointer", fontFamily: "'JetBrains Mono',monospace", fontSize: 11, transition: "all 0.2s" }}>{t}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {debateResult && verdict && (
          <>
            ...
            <div style={{ textAlign: "center", marginBottom: 20, padding: 14, background: "rgba(255,255,255,0.02)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.04)" }}>
              <h2 style={{ fontFamily: "'Sora',sans-serif", fontSize: "1.1em", color: "#fff", margin: 0, fontWeight: 600 }}>{topic}</h2>
            </div>

            {/* FOR vs AGAINST Panels */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
              {/* FOR Panel */}
              <div style={{ background: "rgba(0,255,15,0.02)", border: "1px solid rgba(0,255,15,0.15)", borderRadius: 18, overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: 450 }}>
                <div style={{ background: "rgba(0,255,15,0.06)", padding: "14px 18px", borderBottom: "1px solid rgba(0,255,15,0.1)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
                ...
                <div className="custom-scrollbar" style={{ padding: 16, overflowY: "auto", flex: 1 }}>
                  {forPoints.map((point, i) => (
                    <div key={i} style={{ background: "rgba(0,255,15,0.04)", border: "1px solid rgba(0,255,15,0.08)", borderRadius: 10, padding: "10px 14px", marginBottom: 8, color: "#c8d6e5", fontFamily: "'Hanken Grotesk',sans-serif", fontSize: 13, lineHeight: 1.7 }}>
                      <span style={{ color: C.green, fontWeight: 700, marginRight: 6 }}>•</span>{point}
                    </div>
                  ))}
              ...
              {/* AGAINST Panel */}
              <div style={{ background: "rgba(255,32,64,0.02)", border: "1px solid rgba(255,32,64,0.15)", borderRadius: 18, overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: 450 }}>
                <div style={{ background: "rgba(255,32,64,0.06)", padding: "14px 18px", borderBottom: "1px solid rgba(255,32,64,0.1)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div className="custom-scrollbar" style={{ padding: 16, overflowY: "auto", flex: 1 }}>
                  {againstPoints.map((point, i) => (
                    <div key={i} style={{ background: "rgba(255,32,64,0.04)", border: "1px solid rgba(255,32,64,0.08)", borderRadius: 10, padding: "10px 14px", marginBottom: 8, color: "#c8d6e5", fontFamily: "'Hanken Grotesk',sans-serif", fontSize: 13, lineHeight: 1.7 }}>
                      <span style={{ color: C.crimson, fontWeight: 700, marginRight: 6 }}>•</span>{point}
                    </div>
                  ))}
            ...
            <div style={{ textAlign: "center", padding: 28, background: verdict.winner === "FOR" ? "rgba(0,255,15,0.04)" : verdict.winner === "AGAINST" ? "rgba(255,32,64,0.04)" : "rgba(168,85,247,0.04)", border: `1px solid ${verdict.winner==="FOR"?C.green:verdict.winner==="AGAINST"?C.crimson:C.purple}30`, borderRadius: 18, marginBottom: 24 }}>
              ...
              <div style={{ fontFamily: "'Sora',sans-serif", fontSize: "1.6em", fontWeight: 900, color: verdict.winner === "FOR" ? C.green : verdict.winner === "AGAINST" ? C.crimson : C.purple, marginBottom: 8 }}>
                🏆 {verdict.winner} POSITION WINS
              </div>
              {verdict.reasoning && (
                <div style={{ fontFamily: "'Hanken Grotesk',sans-serif", fontSize: 13, color: C.textSecondary, lineHeight: 1.7, maxWidth: 600, margin: "0 auto", fontStyle: "italic" }}>
                  {verdict.reasoning}
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center", marginBottom: 40 }}>
              <button onClick={handleNewDebate} style={{ padding: "14px 28px", borderRadius: 30, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", color: "#ccc", cursor: "pointer", fontFamily: "'JetBrains Mono',monospace", fontSize: 13, transition: "all 0.2s" }}>🔄 New Debate</button>
              <button onClick={() => navigator.clipboard.writeText(JSON.stringify({ topic, forPoints, againstPoints, verdict }, null, 2))} style={{ padding: "14px 28px", borderRadius: 30, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", color: "#ccc", cursor: "pointer", fontFamily: "'JetBrains Mono',monospace", fontSize: 13 }}>📋 Copy Result</button>
              {onStartResearch && (
                <button onClick={() => onStartResearch(topic)} style={{ padding: "14px 28px", borderRadius: 30, border: "none", background: C.green, color: C.void, fontWeight: 700, cursor: "pointer", fontFamily: "'Sora',sans-serif", fontSize: 13 }}>🔬 Research This Topic →</button>
              )}
            </div>
          </>
        )}
        ...
        {/* Debate History */}
        {history.length > 0 && (
          <div style={{ borderTop: "1px solid " + C.white10, paddingTop: 24 }}>
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: C.textSecondary, textTransform: "uppercase", letterSpacing: "2px", marginBottom: 16 }}>Recent Debates</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
              {history.map((h, i) => (
                <div key={i} onClick={() => { setTopic(h.topic); startDebate({ preventDefault: () => {} }) }} style={{ background: "rgba(10,10,30,0.6)", backdropFilter: "blur(20px)", border: "1px solid " + C.white10, borderRadius: 12, padding: 14, cursor: "pointer", transition: "all 0.2s" }}>
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: h.verdict?.winner === "FOR" ? C.green : C.crimson, marginBottom: 4 }}>{h.verdict?.winner || "?"} WINS</div>
                  <div style={{ fontFamily: "'Hanken Grotesk',sans-serif", fontSize: 13, color: "#ccc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{h.topic}</div>
