...
const REPULSION_STRENGTH = 800, ATTRACTION_STRENGTH = 0.005, DAMPING = 0.85, CENTER_GRAVITY = 0.002;

function runForceSimulation(nodes, edges, width, height, iterations = 60) {
  const simNodes = nodes.map((n, i) => ({
    ...
    size: Math.min(35, Math.max(14, n.size || 20)),
  }));
  for (let iter = 0; iter < iterations; iter++) {
    ...
    edges.forEach(edge => {
      const src = simNodes.find(n => n.id === edge.source), tgt = simNodes.find(n => n.id === edge.target);
      if (!src || !tgt) return;
      const dx = tgt.x - src.x, dy = tgt.y - src.y, dist = Math.sqrt(dx*dx+dy*dy) || 1;
      const force = dist * ATTRACTION_STRENGTH * (edge.weight || 1);
      const fx = (dx/dist)*force, fy = (dy/dist)*force;
      src.vx += fx; src.vy += fy; tgt.vx -= fx; tgt.vy -= fy;
    });
    simNodes.forEach(n => {
      n.vx += (width/2 - n.x) * CENTER_GRAVITY; n.vy += (height/2 - n.y) * CENTER_GRAVITY;
...
function NeuralCanvas() {
  ...
  useEffect(() => {
    ...
    let particles = [], animId; const N = 100;
    ...
    const loop = () => { ctx.clearRect(0,0,canvas.width,canvas.height); particles.forEach((p,i)=>{p.x+=p.vx;p.y+=p.vy;if(p.x<0||p.x>canvas.width)p.vx*=-1;if(p.y<0||p.y>canvas.height)p.vy*=-1;ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,Math.PI*2);ctx.fillStyle=`rgba(168,85,247,${p.opacity})`;ctx.fill();for(let j=i+1;j<particles.length;j++){const d=Math.hypot(p.x-particles[j].x,p.y-particles[j].y);if(d<100){ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(particles[j].x,particles[j].y);ctx.strokeStyle=`rgba(168,85,247,${0.06*(1-d/100)})`;ctx.lineWidth=0.3;ctx.stroke()}}});animId=requestAnimationFrame(loop)};loop();
...
function Sidebar({ onNavigate, user, onLogout }) {
  ...
  const NAV = [
    { icon: "travel_explore", label: "Research", path: "/research", color: C.green },
    { icon: "forum", label: "Debate Chamber", path: "/debate", color: C.crimson },
    { icon: "account_tree", label: "Knowledge Graph", path: "/graph", active: true, color: C.purple },
    { icon: "search", label: "Semantic Search", path: "/search", color: C.cyan },
    { icon: "database", label: "Memory Bank", path: "/memory", color: "#ffaa00" },
    { icon: "picture_as_pdf", label: "PDF Lab", path: "/pdf-lab", color: C.gold },
    { icon: "analytics", label: "Analytics", path: "/analytics", color: C.purple },
  ];
  ...
  if (collapsed) return (
    <aside style={{ position:"fixed",left:0,top:0,height:"100%",width:56,background:"rgba(10,10,30,0.6)",backdropFilter:"blur(24px)",borderRight:"1px solid "+C.white10,display:"flex",flexDirection:"column",alignItems:"center",padding:"16px 0",zIndex:20 }}>
      ...
      <div style={{marginTop:"auto",display:"flex",flexDirection:"column",alignItems:"center",gap:14}}>
        ...
        <div title={user?.username||'Guest'} style={{width:30,height:30,borderRadius:"50%",background:C.surfaceContainer,border:"1px solid rgba(168,85,247,0.3)",display:"flex",alignItems:"center",justifyContent:"center"}}><Icon name="face" style={{color:C.purple,fontSize:14}}/></div>
  ...
  return (
    <aside style={{ position:"fixed",left:0,top:0,height:"100%",width:320,background:"rgba(10,10,30,0.6)",backdropFilter:"blur(24px)",borderRight:"1px solid "+C.white10,boxShadow:"0 0 20px rgba(0,255,15,0.1)",display:"flex",flexDirection:"column",padding:24,zIndex:20,transition:"width 0.35s cubic-bezier(0.4,0,0.2,1),padding 0.35s cubic-bezier(0.4,0,0.2,1)",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:40,minWidth:0}}>
        <div style={{flex:1,minWidth:0}}><h1 style={{fontFamily:"'Sora',sans-serif",fontSize:28,fontWeight:800,color:C.purple,letterSpacing:"-0.03em",whiteSpace:"nowrap"}}>POLYNOUS</h1><p style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:C.onSurfaceVariant,textTransform:"uppercase",letterSpacing:"0.2em",opacity:0.7,whiteSpace:"nowrap"}}>Cerebral Vitality Engine</p></div>
      ...
      <nav style={{flex:1,display:"flex",flexDirection:"column",gap:4,overflow:"hidden"}}>
        {NAV.map(({icon,label,path,active,color})=><div key={label} onClick={()=>handleNav(path)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",borderRadius:9999,cursor:"pointer",color:active?color:C.onSurfaceVariant,background:active?`${color}15`:"transparent",fontFamily:"'JetBrains Mono',monospace",fontSize:13,fontWeight:active?700:400,transition:"all 0.2s",whiteSpace:"nowrap",overflow:"hidden"}} onMouseEnter={e=>{if(!active){e.target.style.color=color;e.target.style.background="rgba(255,255,255,0.05)"}}} onMouseLeave={e=>{if(!active){e.target.style.color=C.onSurfaceVariant;e.target.style.background="transparent"}}}><Icon name={icon} style={{fontSize:20,color:"inherit",flexShrink:0}}/><span style={{overflow:"hidden",textOverflow:"ellipsis"}}>{label}</span></div>)}
      </nav>
      <div style={{borderTop:"1px solid "+C.white5,paddingTop:24,marginTop:24}}>
        <button onClick={()=>handleNav('/research')} style={{width:"100%",padding:"12px",background:C.purple,color:"#fff",fontWeight:700,borderRadius:9999,border:"none",cursor:"pointer",fontFamily:"'Sora',sans-serif",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"transform 0.2s",whiteSpace:"nowrap"}} onMouseEnter={e=>e.target.style.transform="scale(1.03)"} onMouseLeave={e=>e.target.style.transform="scale(1)"}><Icon name="add" style={{fontSize:18,color:"#fff",flexShrink:0}}/>New Research</button>
        <div style={{marginTop:20,display:"flex",alignItems:"center",gap:12}}><div style={{width:40,height:40,borderRadius:"50%",background:C.surfaceContainer,border:"1px solid rgba(168,85,247,0.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon name="face" style={{color:C.purple,fontSize:22}}/></div><div style={{flex:1,minWidth:0}}><p style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,fontWeight:700,color:'#fff',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{user?.username||'Guest'}</p><button onClick={handleLogout} style={{fontSize:10,color:C.crimson,background:'none',border:'none',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",padding:0}}>Disconnect</button></div></div>
      </div>
    </aside>
  );
}
...
export default function KnowledgeGraphPage({ user, onStartResearch, onNavigate, onLogout, graphData: propData }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try { const res = await fetch('http://localhost:8000/knowledge/graph'); if (res.ok) setGraphData(await res.json()); }
  ...
  useEffect(() => {
    ...
    setPositions(runForceSimulation(graphData.nodes, graphData.edges || [], W, H, 60));
  }, [graphData]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      (graphData.edges || []).forEach(edge => {
        ...
        if (!src || !tgt) return;
        const edgeHL = (hovered && (hovered.id === src.id || hovered.id === tgt.id)) || (selectedNode && (selectedNode.id === src.id || selectedNode.id === tgt.id));
        ...
        if (edgeHL || searchMatch) { ctx.strokeStyle = 'rgba(168,85,247,0.4)'; ctx.lineWidth = 1.5; }
        else if (searchQuery) { ctx.strokeStyle = 'rgba(255,255,255,0.02)'; ctx.lineWidth = 0.3; }
        else { ctx.strokeStyle = `rgba(160,155,210,${0.04+(edge.weight||1)*0.02})`; ctx.lineWidth = 0.5; }
        ctx.stroke();
      });
      positions.forEach(n => {
        ...
        const isDimmed = (searchQuery && !matchesSearch) || (!!selectedNode && !isSel);
        const pulse = Math.sin(Date.now()*0.003+(n.id?.charCodeAt(0)||0))*1.5;
        const r = n.size + pulse + (isHov||isSel?5:0);
        const col = NODE_COLORS[n.type] || NODE_COLORS.default;
        ctx.globalAlpha = isDimmed ? 0.15 : 1;
        const auraR = r + (isSel?28:isHov?22:10);
        const aura = ctx.createRadialGradient(n.x,n.y,r*0.3,n.x,n.y,auraR);
        aura.addColorStop(0,`rgba(${hexToRgb(col.glow).join(',')},${isSel?0.4:isHov?0.25:0.06})`);
        ...
        ctx.beginPath();ctx.arc(n.x,n.y,r,0,Math.PI*2);ctx.strokeStyle=isSel?'#fff':isHov?'rgba(255,255,255,0.6)':'rgba(255,255,255,0.2)';ctx.lineWidth=isSel?2:isHov?1.5:0.5;ctx.stroke();
        ctx.globalAlpha=1;
        const label=(n.label?.length>16?n.label.slice(0,15)+'…':n.label)||'';
        ctx.font=`${isHov||isSel?600:400} ${isHov||isSel?11:10}px 'JetBrains Mono',monospace`;ctx.textAlign='center';
        ctx.fillStyle=(isHov||isSel||matchesSearch)?'#fff':(col.text||'#aac4ff');
        ctx.fillText(label,n.x,n.y+r+14);
        if(isHov&&n.connections>0){ctx.font="500 9px 'JetBrains Mono',monospace";ctx.fillText(`${n.connections} links`,n.x,n.y-r-8);}
      });
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    setHovered(found || null); canvasRef.current.style.cursor = found ? "pointer" : "default";
  }, [positions]);
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',background:C.void,display:'flex',alignItems:'center',justifyContent:'center',position:'relative'}}>
      ...
      <div style={{textAlign:'center',zIndex:10}}>
        <div style={{width:50,height:50,borderRadius:'50%',border:'3px solid rgba(168,85,247,0.15)',borderTop:`3px solid ${C.purple}`,animation:'spin 1s infinite',margin:'0 auto 20px'}}/>
        <div style={{fontFamily:"'Sora',sans-serif",color:C.purple,fontWeight:600,fontSize:15}}>Loading Neural Topology</div>
      </div>
  ...
  return (
    <div style={{minHeight:'100vh',background:C.void,position:'relative',overflow:'hidden'}}>
      ...
      <div style={{marginLeft:320,position:'relative',zIndex:10,height:'100vh'}}>
        <canvas ref={canvasRef} style={{width:'100%',height:'100%',display:'block'}} onMouseMove={handleMouseMove} onClick={handleCanvasClick} />
        ...
        <div style={{position:'absolute',top:50,left:12,zIndex:20}}>
          <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 14px',borderRadius:25,background:'rgba(10,8,22,0.85)',border:'0.5px solid rgba(255,255,255,0.15)',backdropFilter:'blur(10px)'}}>
            ...
            <input type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search nodes..." style={{background:'none',border:'none',outline:'none',color:'#fff',fontSize:12,fontFamily:"'JetBrains Mono',monospace",width:160}}/>
          ...
          {searchQuery&&<div style={{fontSize:10,color:C.textSecondary,fontFamily:"'JetBrains Mono',monospace",marginTop:4,marginLeft:4}}>{positions.filter(n=>n.label.toLowerCase().includes(searchQuery.toLowerCase())).length} matches</div>}
        </div>

        {/* Filter pills */}
        <div style={{position:'absolute',top:12,left:12,display:'flex',gap:6,flexWrap:'wrap'}}>
          {filterTypes.map(t=>(<button key={t} onClick={()=>setFilter(t)} style={{background:filter===t?'rgba(168,85,247,0.15)':'rgba(255,255,255,0.04)',border:'0.5px solid rgba(255,255,255,0.15)',borderRadius:20,padding:'5px 13px',fontSize:11,color:filter===t?'#fff':'rgba(255,255,255,0.45)',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",textTransform:'capitalize'}}>{t}</button>))}
        </div>
        ...
        <div style={{position:'absolute',bottom:14,right:14,display:'flex',flexDirection:'column',gap:6,background:'rgba(10,8,20,0.7)',border:'0.5px solid rgba(255,255,255,0.1)',borderRadius:10,padding:'10px 14px'}}>
          {["concept","entity","topic","core","major","debate"].filter(t=>NODE_COLORS[t]).map(type=>(<div key={type} style={{display:'flex',alignItems:'center',gap:8}}><span style={{width:8,height:8,borderRadius:'50%',background:NODE_COLORS[type]?.fill,flexShrink:0}}/><span style={{fontSize:10,color:'rgba(255,255,255,0.4)',fontFamily:"'JetBrains Mono',monospace",textTransform:'uppercase'}}>{type}</span></div>))}
        </div>

        {/* Metrics Panel */}
        <div style={{position:'absolute',top:12,right:12,width:260,display:'flex',flexDirection:'column',gap:10}}>
          {selectedNode&&nodeDetails?(
            <div style={{background:'rgba(10,8,22,0.92)',border:'0.5px solid rgba(168,85,247,0.3)',borderRadius:12,padding:'16px'}}>
              ...
              <MetricRow label="Connections" value={nodeDetails.total_connections||0} color={C.purple}/>
              {nodeResearch.length>0&&(
                <div style={{marginTop:10,paddingTop:10,borderTop:'1px solid rgba(255,255,255,0.08)'}}>
                  ...
                  {nodeResearch.map((r,i)=>(<div key={i} onClick={()=>onStartResearch?.(r.query)} style={{fontSize:10,color:C.onSurfaceVariant,padding:'3px 0',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",opacity:0.7}}>🔬 {r.query?.slice(0,50)}</div>))}
                </div>
              )}
              <button onClick={()=>onStartResearch?.(selectedNode.label)} style={{width:'100%',marginTop:12,padding:'8px',borderRadius:8,border:'none',background:C.purple,color:'#fff',fontWeight:700,cursor:'pointer',fontSize:12,fontFamily:"'Sora',sans-serif"}}>Research This →</button>
            </div>
          ):(
      ...
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=JetBrains+Mono:wght@400;500&family=Hanken+Grotesk:wght@400;500;600&family=Material+Symbols+Outlined&display=swap');@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    </div>
  );
}